...
function CurrentMentor() {
  ...
  const handleClose = () => {
    axios
      .post(`/api/user/mentee/close_connection/`, {
        username: currentMentor.username,
      })
      .then((res) => {
  ...
  useEffect(() => {
    axios
      .get("/api/user/mentee/get_current_mentor/", {
        headers: {
          Authorization: `Token ${token}`,
        },
      })
      .then((res) => {
        setcurrentMentor(res.data[0].mentor);
      })
      .catch((err) => {
        console.log(err);
      });
        ...
        <div className="w-3/4 mx-auto">
          {currentMentor ? (
                            ...
                            <tr key={1}>
                              <td className="px-6 py-4 whitespace-nowrap w-2/12 dark:bg-gray-800">
                                <div className="text-sm font-medium text-gray-900 dark:text-gray-200">
                                  {currentMentor.first_name}{" "}
                                  {currentMentor.last_name}
                                </div>
                              </td>
                              ...
                              <td className="px-6 py-4 whitespace-nowrap w-1/12 dark:bg-gray-800">
                                ...
                                >
                                  ...
                                  <span className="mx-1">Close Connection</span>
                                </button>
                              </td>
                            </tr>
                          </tbody>
            ...
            <p className="text-center text-gray-700 dark:text-gray-200 font-bold text-md m-2 p-2">
              You do not have any current mentor!
            </p>
          )}
        </div>
      </div>
